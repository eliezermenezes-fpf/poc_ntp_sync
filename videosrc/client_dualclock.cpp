...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        std::cerr
            << "Uso: " << argv[0]
            << " <modo> [rtsp_url] [clock_host] [clock_port]\n"
            << "  modo: 'net' ou 'sys'\n"
            << "\n"
            << "Parâmetros:\n"
            << "  rtsp_url   = URL RTSP (ex.: rtsp://HOST:8555/live)\n"
            << "  clock_host = host/IP onde o GstNetTimeProvider está (normalmente o mesmo HOST do RTSP)\n"
            << "  clock_port = porta UDP do clock (ex.: 8557)\n"
            << "\n"
            << "Exemplos:\n"
            << "  " << argv[0] << " net rtsp://127.0.0.1:8555/live 127.0.0.1 8557\n"
            << "  " << argv[0] << " sys rtsp://127.0.0.1:8555/live 127.0.0.1 8557\n";
        return 1;
    }

    std::string mode = argv[1];
    if (mode != "net" && mode != "sys") {
        std::cerr << "Modo inválido: " << mode << " (use 'net' ou 'sys').\n";
        return 1;
    }

    const char* rtsp_url   = (argc > 2) ? argv[2] : "rtsp://127.0.0.1:8555/live";
    ...
    // 1) Clock do servidor (NetClientClock) – sempre criado para medir tempo remoto
    ...
    // IMPORTANTE: aqui vai a URL RTSP (NÃO é caminho de arquivo .mp4)
    g_object_set(src,
                 "location", rtsp_url,
                 NULL);
    ...
    // 3) Clock local do sistema (pra comparar e opcionalmente usar no pipeline)
    GstClock* sys_clock = gst_system_clock_obtain();

    // 4) Escolhe qual clock o pipeline vai usar
    if (mode == "net") {
        ...
        std::cout << "[CLIENT] Pipeline usando SYS clock (local)." << std::endl;
    }
    ...
    // 6) Thread de sync – UM PRINT por segundo com net/sys/delta/pos
    std::thread syncThread([&]() {
        while (running) {
            std::this_thread::sleep_for(std::chrono::seconds(1));

            GstClockTime net_t = gst_clock_get_time(net_clock);   // tempo remoto
    ...
    // Só pra logar o tipo de clock do pipeline
    {
        ...
        if (pipeline_clock) {
            ...
            std::cout << "[CLIENT] Tipo do sys_clock:         "
                      << G_OBJECT_TYPE_NAME(sys_clock) << std::endl;
            std::cout << "[CLIENT] Pipeline usando net_clock? "
                      << (pipeline_clock == net_clock ? "SIM" : "NAO") << std::endl;
            std::cout << "[CLIENT] Pipeline usando sys_clock? "
                      << (pipeline_clock == sys_clock ? "SIM" : "NAO") << std::endl;
