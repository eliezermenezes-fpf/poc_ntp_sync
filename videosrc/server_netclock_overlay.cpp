...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[SERVER] Clock UDP em 0.0.0.0:" << CLOCK_PORT << std::endl;

    // 1) Clock mestre (sem calibração, sem drift)
    GstClock* clock = gst_system_clock_obtain();

    // 2) NetTimeProvider fixo em 0.0.0.0:CLOCK_PORT
    std::cout << "[SERVER] Criando GstNetTimeProvider em 0.0.0.0:" << CLOCK_PORT << std::endl;
    ...
    // 3) Pipeline:
    // filesrc → decodebin → queue → videoconvert → timeoverlay → videoscale
    //         → identity(sync=true) → x264enc → rtspclientsink
    ...
    if (!pipeline || !filesrc || !decode || !queue || !convert ||
        !overlay || !scale || !ident || !enc || !sink) {
    ...
    // overlay: tempo do fluxo (padrão), canto inferior direito
    g_object_set(overlay,
                 "halign", 2,   // right
                 "valign", 2,   // bottom
                 NULL);

    // pacing em tempo real
    g_object_set(ident,
                 "sync", TRUE,
                 NULL);
    ...
    gst_bin_add_many(GST_BIN(pipeline),
                     filesrc, decode, queue,
                     convert, overlay, scale, ident,
                     enc, sink,
                     NULL);
    ...
    if (!gst_element_link_many(queue, convert, overlay, scale, ident, enc, sink, NULL)) {
    ...
    // 4) Pipeline usa o MESMO clock que estamos expondo (sem drift)
